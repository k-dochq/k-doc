...
> ⚠️ **2026-06-10 정식 PRD(`K-DOC 상담 챗봇 PRD`) 반영 — 진입 UX 전면 재구성 결정**
> 아래 Phase 1(UI-03 카테고리 칩 등) 일부는 **재작업 대상**이 됨. 변경 요지(요구사항 §12.9 정본):
> - 카테고리 칩 4종 → **메인 메뉴 4버튼**(병원 추천 받기 / K-DOC 서비스 안내 / K-DOC 컨시어지 서비스 / 기타 문의)
> - **K-DOC 서비스 안내 = 버튼형 FAQ 서브트리**(소개/검증기준/예약절차/컨시어지) 신규
> - 국적: 자유텍스트 → **드롭다운/검색 선택**
> - **어드민 CMS**(모든 문구/메뉴/버튼 관리) 신규 — 이번 범위 포함
> - **DRAFT/SUBMITTED 상태 + 화면별 뒤로가기/X닫기/네트워크 재시도** 신규
> - 백엔드(`KdocChatThread`+realtime+운영시간 외 안내+슬랙/이메일)는 **재사용**, 응대 모델은 실시간 채팅 단일(운영시간 외 = 접수 안내문만)
