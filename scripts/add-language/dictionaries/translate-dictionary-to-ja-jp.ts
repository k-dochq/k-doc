...
const JA_JSON_FILE = path.join(__dirname, '../../../app/[lang]/dictionaries/ja.json');
...
function saveDictionary(result: Record<string, unknown>): void {
  const outputDir = path.dirname(JA_JSON_FILE);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  fs.writeFileSync(JA_JSON_FILE, JSON.stringify(result, null, 2));
}

/**
 * 메인 번역 함수
 */
async function translateDictionaryToJaJP() {
  try {
    console.log('🌐 다국어 사전 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 기존 ja.json 파일이 있으면 로드
    let jaData: Record<string, unknown> = {};
    if (fs.existsSync(JA_JSON_FILE)) {
      try {
        jaData = JSON.parse(fs.readFileSync(JA_JSON_FILE, 'utf-8'));
        console.log('📁 기존 일본어 사전 파일 로드 완료');
      } catch (error) {
        console.warn('⚠️  기존 일본어 사전 파일을 읽는 중 오류 발생, 새로 생성합니다.');
      }
    }
    ...
    const pathsToTranslate = allPaths.filter((path) => {
      ...
      // 기존 ja 데이터에 값이 있으면 제외
      const existingValue = getValueByPath(jaData, path);
    ...
    // ja 데이터 초기화 (ko 데이터 구조 복사)
    if (Object.keys(jaData).length === 0) {
      jaData = JSON.parse(JSON.stringify(koData));
    }
    ...
    while (processedCount < pathsToTranslate.length) {
      ...
      for (let i = 0; i < batch.length; i += TRANSLATION_BATCH_SIZE) {
        ...
        try {
          console.log(
            `  🔄 일본어 번역 중... (ko → ja) (${i + 1}-${Math.min(i + TRANSLATION_BATCH_SIZE, translationBatch.length)}/${translationBatch.length})`,
          );
          const jaTranslations = await translateBatch(texts, 'ko', 'ja');

          // 결과 저장
          translationBatch.forEach((path, index) => {
            setValueByPath(jaData, path, jaTranslations[index]);
          ...
          for (const path of translationBatch) {
            try {
              ...
              if (text) {
                const jaTranslation = await translateText(text, 'ko', 'ja');
                setValueByPath(jaData, path, jaTranslation);
                progress.processedKeys.push(path);
                await new Promise((resolve) => setTimeout(resolve, 100));
              }
    ...
    console.log('\n🎉 모든 다국어 사전 일본어 번역 완료!');
    console.log(`📁 결과 파일: ${JA_JSON_FILE}`);
...
export { translateDictionaryToJaJP };
