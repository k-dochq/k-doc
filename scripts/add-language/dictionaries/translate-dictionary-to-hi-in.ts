...
const HI_JSON_FILE = path.join(__dirname, '../../../app/[lang]/dictionaries/hi.json');
...
function saveDictionary(result: Record<string, unknown>): void {
  const outputDir = path.dirname(HI_JSON_FILE);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  fs.writeFileSync(HI_JSON_FILE, JSON.stringify(result, null, 2));
}

/**
 * 메인 번역 함수
 */
async function translateDictionaryToHiIN() {
  try {
    console.log('🌐 다국어 사전 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 기존 hi.json 파일이 있으면 로드
    let hiData: Record<string, unknown> = {};
    if (fs.existsSync(HI_JSON_FILE)) {
      try {
        hiData = JSON.parse(fs.readFileSync(HI_JSON_FILE, 'utf-8'));
        console.log('📁 기존 힌디어 사전 파일 로드 완료');
      } catch (error) {
        console.warn('⚠️  기존 힌디어 사전 파일을 읽는 중 오류 발생, 새로 생성합니다.');
      }
    }
    ...
    const pathsToTranslate = allPaths.filter((path) => {
      ...
      // 기존 hi 데이터에 값이 있으면 제외
      const existingValue = getValueByPath(hiData, path);
    ...
    // hi 데이터 초기화 (ko 데이터 구조 복사)
    if (Object.keys(hiData).length === 0) {
      hiData = JSON.parse(JSON.stringify(koData));
    }
    ...
    while (processedCount < pathsToTranslate.length) {
      ...
      for (let i = 0; i < batch.length; i += TRANSLATION_BATCH_SIZE) {
        ...
        try {
          console.log(
            `  🔄 힌디어 번역 중... (ko → hi) (${i + 1}-${Math.min(i + TRANSLATION_BATCH_SIZE, translationBatch.length)}/${translationBatch.length})`,
          );
          const hiTranslations = await translateBatch(texts, 'ko', 'hi');

          // 결과 저장
          translationBatch.forEach((path, index) => {
            setValueByPath(hiData, path, hiTranslations[index]);
          ...
          for (const path of translationBatch) {
            try {
              ...
              if (text) {
                const hiTranslation = await translateText(text, 'ko', 'hi');
                setValueByPath(hiData, path, hiTranslation);
                progress.processedKeys.push(path);
                await new Promise((resolve) => setTimeout(resolve, 100));
              }
    ...
    console.log('\n🎉 모든 다국어 사전 힌디어 번역 완료!');
    console.log(`📁 결과 파일: ${HI_JSON_FILE}`);
...
export { translateDictionaryToHiIN };
