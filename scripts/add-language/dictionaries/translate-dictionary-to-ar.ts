/**
 * 한국어 사전(ko.json)을 Google Translate API로 아랍어(ar.json)로 번역하는 스크립트.
 * - 소스: ko (한국어), 대상: ar (아랍어).
 * - 실행: npx ts-node scripts/add-language/dictionaries/translate-dictionary-to-ar.ts
 * - ar.json이 en 복사본 등으로 이미 영어로 채워져 있으면 "번역 필요 0개"로 끝나므로,
 *   아랍어로 전량 재번역하려면 반드시 --force 옵션으로 실행:
 *   npx ts-node scripts/add-language/dictionaries/translate-dictionary-to-ar.ts --force
 */
...
const AR_JSON_FILE = path.join(__dirname, '../../../app/[lang]/dictionaries/ar.json');
const PROGRESS_FILE = path.join(__dirname, 'output/translation-progress-ar.json');
...
function saveDictionary(result: Record<string, unknown>): void {
  const outputDir = path.dirname(AR_JSON_FILE);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  fs.writeFileSync(AR_JSON_FILE, JSON.stringify(result, null, 2));
}

/**
 * 메인 번역 함수
 * @param force - true이면 기존 ar.json 값/진행상황을 무시하고 ko → ar 전부 재번역
 */
async function translateDictionaryToAr(force = false) {
  try {
    console.log('🌐 다국어 사전 아랍어(ar) 번역 작업 시작...');
    if (force) {
      console.log('⚠️  --force: 기존 값·진행상황 무시 후 전량 ko → ar 재번역');
    }
    ...
    // 기존 ar.json 파일이 있으면 로드 (force가 아닐 때만 활용)
    let arData: Record<string, unknown> = {};
    if (!force && fs.existsSync(AR_JSON_FILE)) {
      try {
        arData = JSON.parse(fs.readFileSync(AR_JSON_FILE, 'utf-8'));
        console.log('📁 기존 아랍어 사전 파일 로드 완료');
      } catch (error) {
        console.warn('⚠️  기존 아랍어 사전 파일을 읽는 중 오류 발생, 새로 생성합니다.');
      }
    }
    ...
    const pathsToTranslate = allPaths.filter((pathStr) => {
      ...
      // 기존 ar 데이터에 값이 있으면 제외
      const existingValue = getValueByPath(arData, pathStr);
    ...
    // ar 데이터 초기화: 비어있거나 force면 ko 구조 복사 (한국어 원문으로 덮어쓰기)
    if (Object.keys(arData).length === 0 || force) {
      arData = JSON.parse(JSON.stringify(koData));
      if (force) {
        console.log('📁 아랍어 사전을 한국어 구조로 초기화 (ko → ar 번역으로 채움)');
      }
    }
    ...
    while (processedCount < pathsToTranslate.length) {
      ...
      for (let i = 0; i < batch.length; i += TRANSLATION_BATCH_SIZE) {
        ...
        try {
          console.log(
            `  🔄 아랍어 번역 중... (ko → ar) (${i + 1}-${Math.min(i + TRANSLATION_BATCH_SIZE, translationBatch.length)}/${translationBatch.length})`,
          );
          const arTranslations = await translateBatch(texts, 'ko', 'ar');

          // 결과 저장
          translationBatch.forEach((pathStr, index) => {
            setValueByPath(arData, pathStr, arTranslations[index]);
          ...
          for (const pathStr of translationBatch) {
            try {
              ...
              if (text) {
                const arTranslation = await translateText(text, 'ko', 'ar');
                setValueByPath(arData, pathStr, arTranslation);
                progress.processedKeys.push(pathStr);
                await new Promise((resolve) => setTimeout(resolve, 100));
              }
    ...
    console.log('\n🎉 모든 다국어 사전 아랍어 번역 완료!');
    console.log(`📁 결과 파일: ${AR_JSON_FILE}`);
...
// 스크립트 실행 (--force: 기존 ar 값 무시하고 ko → ar 전량 재번역)
if (require.main === module) {
  const force = process.argv.includes('--force');
  translateDictionaryToAr(force)
    .then(() => {
...
export { translateDictionaryToAr };
