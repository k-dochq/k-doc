...
const TL_JSON_FILE = path.join(__dirname, '../../../app/[lang]/dictionaries/tl.json');
const PROGRESS_FILE = path.join(__dirname, 'output/translation-progress-tl.json');
...
function saveDictionary(result: Record<string, unknown>): void {
  const outputDir = path.dirname(TL_JSON_FILE);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  fs.writeFileSync(TL_JSON_FILE, JSON.stringify(result, null, 2));
}

/**
 * 메인 번역 함수
 */
async function translateDictionaryToTl() {
  try {
    console.log('🌐 다국어 사전 필리핀어(tl) 번역 작업 시작...');
    ...
    // 기존 tl.json 파일이 있으면 로드
    let tlData: Record<string, unknown> = {};
    if (fs.existsSync(TL_JSON_FILE)) {
      try {
        tlData = JSON.parse(fs.readFileSync(TL_JSON_FILE, 'utf-8'));
        console.log('📁 기존 필리핀어 사전 파일 로드 완료');
      } catch (error) {
        console.warn('⚠️  기존 필리핀어 사전 파일을 읽는 중 오류 발생, 새로 생성합니다.');
      }
    }
    ...
    const pathsToTranslate = allPaths.filter((path) => {
      ...
      // 기존 tl 데이터에 값이 있으면 제외
      const existingValue = getValueByPath(tlData, path);
    ...
    // tl 데이터 초기화 (ko 데이터 구조 복사)
    if (Object.keys(tlData).length === 0) {
      tlData = JSON.parse(JSON.stringify(koData));
    }
    ...
    while (processedCount < pathsToTranslate.length) {
      ...
      for (let i = 0; i < batch.length; i += TRANSLATION_BATCH_SIZE) {
        ...
        try {
          console.log(
            `  🔄 필리핀어 번역 중... (ko → tl) (${i + 1}-${Math.min(i + TRANSLATION_BATCH_SIZE, translationBatch.length)}/${translationBatch.length})`,
          );
          const tlTranslations = await translateBatch(texts, 'ko', 'tl');

          // 결과 저장
          translationBatch.forEach((path, index) => {
            setValueByPath(tlData, path, tlTranslations[index]);
          ...
          for (const path of translationBatch) {
            try {
              ...
              if (text) {
                const tlTranslation = await translateText(text, 'ko', 'tl');
                setValueByPath(tlData, path, tlTranslation);
                progress.processedKeys.push(path);
                await new Promise((resolve) => setTimeout(resolve, 100));
              }
    ...
    console.log('\n🎉 모든 다국어 사전 필리핀어 번역 완료!');
    console.log(`📁 결과 파일: ${TL_JSON_FILE}`);
...
export { translateDictionaryToTl };
