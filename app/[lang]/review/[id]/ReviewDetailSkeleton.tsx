  ...
  return (
    <div className=''>
      ...
      <div className='flex items-center justify-between border-b border-neutral-100 px-5 py-4'>
        ...
        {/* 좋아요 버튼 */}
        <div className='h-8 w-16 animate-pulse rounded-full bg-white/50'></div>
      </div>
      ...
      <div className='px-5 py-4'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center space-x-3'>
            {/* 프로필 사진 */}
            <div className='h-10 w-10 animate-pulse rounded-full bg-white/50'></div>
          ...
          <div className='flex items-center space-x-1'>
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className='h-4 w-4 animate-pulse rounded bg-white/50'></div>
            ))}
          </div>
        </div>
        ...
        <div className='mt-3 space-y-2'>
          <div className='grid grid-cols-2 gap-2'>
            {/* Before 이미지 */}
            <div className='aspect-[4/3] animate-pulse rounded-lg bg-white/50'></div>
            {/* After 이미지 */}
            <div className='aspect-[4/3] animate-pulse rounded-lg bg-white/50'></div>
          </div>
          {/* 추가 이미지 인디케이터 */}
          <div className='flex justify-center space-x-1'>
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className='h-2 w-2 animate-pulse rounded-full bg-white/50'></div>
            ))}
          </div>
        </div>
        ...
        <div className='mt-3 space-y-2'>
          ...
          <div className='flex flex-wrap gap-2'>
            ...
            <div className='h-6 w-14 animate-pulse rounded-full bg-white/50'></div>
          </div>
          {/* 시술시기 */}
        ...
        <div className='mt-3 rounded-lg border border-neutral-200 p-4'>
          <div className='flex items-center space-x-3'>
            {/* 병원 이미지 */}
            <div className='h-12 w-12 animate-pulse rounded-lg bg-white/50'></div>
      ...
      <div className='p-5'>
        {/* 섹션 제목 */}
        <div className='mb-4 h-6 w-32 animate-pulse rounded bg-white/50'></div>
        ...
        <div className='space-y-4'>
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className='rounded-lg border border-neutral-200 p-4'>
              ...
              {/* 이미지 */}
              <div className='mb-3 aspect-[4/3] animate-pulse rounded-lg bg-white/50'></div>
