...
export function useInfiniteSearchReviews({ query, limit = 10, categories }: UseInfiniteSearchReviewsParams) {
  return useInfiniteQuery({
    queryKey: ['reviews', 'v2', 'search', 'infinite', { query, limit, categories }],
    queryFn: ({ pageParam }) => fetchSearchReviews({ query, page: pageParam, limit, categories }),
    initialPageParam: 1,
