...
import { ReviewSortFilterDrawer } from './ReviewSortFilterDrawer';
...
export function ReviewFilterBarV2({ lang, dict, currentSort, hospitalId }: ReviewFilterBarV2Props) {
  ...
  const getSortLabel = (): string => {
    switch (currentSort) {
      ...
      case REVIEW_SORT_OPTIONS.RATING_LOW:
        return dict.allReviews?.sort?.ratingLow || '별점 낮은 순';
      default:
        return dict.allReviews?.sort?.popular || '인기순';
    }
  };
      ...
      content: (
        <ReviewSortFilterDrawer
          ...
          onSelect={(sort) => {
            ...
            router.replace(`/v2/hospital/${hospitalId}/reviews?${params.toString()}`);
          }}
        />
      ),
    });
